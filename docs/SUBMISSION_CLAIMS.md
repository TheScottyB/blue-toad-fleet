...
|---|---|---|---|
| Source cycle identity | Manifest plus every source object's SHA-256, staged and materialized by `src/cycles/storage.py` | Implementation verified; Aug-22 output sealed at `data/aug22_gallery_4160518/artifact_manifest.json` (`333ea63`) | “Sanctioned, hash-bound gallery drop.” The sealed Aug-22 artifacts are submission evidence for that cycle; they are not a later sale. |
...
| Screenshots and walkthrough | Challenge/landing-page validation, expected marker, staged captures, sealed facts SHA | Capture tooling verified; fresh beat recordings staged 2026-08-29; checked-in stills in `docs/screenshots/` remain historical | Label stills historical. The final MP4 row below is the video claim. |
| Final MP4 | Canonical assembler, release-eligible facts, declared input hashes, embedded facts/input digest, media validation | Built 2026-08-29 by the canonical assembler from release-eligible facts (sealed artifact manifest `333ea63`); under the 4:00 cap, facts/input digest embedded, `make video-verify` passes | The checked-in MP4 is current submission evidence while `make video-verify` passes against it. |
